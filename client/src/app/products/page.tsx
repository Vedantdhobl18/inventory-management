...
const Products = () => {
    ...
    const [isModalOpen , setisModalOpen] = useState(false);
     ...
     const handleCreateProduct = async (productsData : productFromData) => {
        await createProduct(productsData);
         ...
         <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 justify-between'>
            {isLoading ? ( <div>Loading...</div> ) : (
                products.map ((product) => (
                    <div key={product.productId} className='border shadow rounded-md p-4 max-w-full w-full max-auto'>
